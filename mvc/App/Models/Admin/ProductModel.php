...
class ProductModel extends \Core\Model
{
	...
	public static function addProduct($dataArray){
		extract($dataArray);
		$urlkey = ProductModel::getProductURLKey($txtProductName);
		if(!$urlkey){
			return false;
		}
		else{
			$txtProductName = ProductModel::getProductNameStudlyCase($txtProductName);
			...
			try{
				...
				$txtProductDescription = addslashes($txtProductDescription);
				$txtProductShortDescription = addslashes($txtProductShortDescription);
	...
	public static function getProductDetails($url_key){
		try{
            $db = static::getDB();
            ...
            $stmt = $db->query("SELECT 
            						...
            						products_price,  
            						products_stock 
            					FROM products WHERE products_urlkey='$url_key'");
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
